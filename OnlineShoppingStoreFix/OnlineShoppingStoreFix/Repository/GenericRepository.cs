...
namespace OnlineShoppingStore.Repository
{
    public class GenericRepository<Tbl_Entity> : IRepository<Tbl_Entity> where Tbl_Entity : class
    {
        ...
        public IEnumerable<Tbl_Entity> GetRecordsToShow(int PageNo, int PageSize, int CurrentPage, Expression<Func<Tbl_Entity, bool>> wherePredict, Expression<Func<Tbl_Entity, int>> orderByPrediction)
        {
            if (wherePredict != null)
            {
                return _dbSet.OrderBy(orderByPrediction).Where(wherePredict).ToList();
            }
            else
            {
                return _dbSet.OrderBy(orderByPrediction).Where(wherePredict).ToList();
        ...
        public IEnumerable<Tbl_Entity> GetCategoy()
        {
            return _dbSet.ToList();
        }
    }
